...
RTC edge node or media gateway
  ↕
AI model or voice-agent server
```
...
Media processing and transcoding
  ↓
CDN or low-latency live-streaming network
  ↓↓↓↓↓
Large audience
```
...
```
Host + multiple active speakers
          ↕
   SFU or audio-mixing server
          ↓
       Room listeners
```
